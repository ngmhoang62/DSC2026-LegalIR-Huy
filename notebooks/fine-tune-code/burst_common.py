...
KAGGLE_ROOT = Path("/kaggle/input/fine-tune-ref/ref")   # dataset nhtclone/fine-tune-ref
...
def default_root():
    ...
    both from the repo and from /kaggle/input/fine-tune-ref/ref/fine_tune/.
    """
...
def load_queries(root=ROOT):
    path = Path(root) / DATA_SUBDIR / "train.json"
...
class RunRecorder:
    ...
    Only epochs that beat the running best are persisted, so the folder always
    holds the single state worth submitting plus the full history to justify it.
    """
    ...
    def __init__(self, work, tag, baseline, select=("recall", "f2"), resume=True):
        ...
        self.best_key = self._key(baseline)
        self.best_epoch = None          # None = nothing has beaten the cache yet
    ...
    def _resume(self):
        ...
        for row in rows:
            if row.get("improved") and all(name in row for name in self.select):
                self.best_key = max(self.best_key, self._key(row))
        ...
        print(f"[{self.tag}] resuming: epochs {self.done_epochs or '-'} already "
              ...
              f"best {self.select[0]}={self.best_key[0]:.4f}", flush=True)
    ...
    def consider(self, epoch, metrics, state_dict, ranked, predictions,
        ...
        row = {"epoch": epoch, "improved": improved, **dict(metrics)}
        if extra:
            row.update(extra)
        ...
        print(f"[{self.tag}] epoch {epoch}: {self.select[0]}={metrics[self.select[0]]:.4f} "
              ...
              f"-> {'SAVED (new best)' if improved else 'not saved'}", flush=True)
        return improved
