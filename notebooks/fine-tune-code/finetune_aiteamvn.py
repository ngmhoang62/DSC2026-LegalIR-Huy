...
negatives mined from the cached lexical pool, plus the other groups' documents
in the same batch.
...
def load_model(root, model_path, gradient_checkpointing):
    ...
    # form and would not train stably.
...
def run(args, tag, channel, load, device=None, amp_dtype=None):
    ...
    if recorder.best_epoch is None:
        print("No epoch beat the cached baseline -- keep the shipped weights.\n"
