...
0-749 and 850-1149).  Hard negatives are the top of each query's cached lexical
pool minus the gold documents, so the model trains against the exact distractors
the deployed retrieval produces.
...
def load_model(root, model_path, init_checkpoint, gradient_checkpointing):
    ...
    # for inference directly would not converge.
    try:
        model = AutoModelForSequenceClassification.from_pretrained(
            source, trust_remote_code=True, use_flash_attn=False)
    except TypeError:
        model = AutoModelForSequenceClassification.from_pretrained(
...
def main():
    ...
    examples = bc.build_train_pool(args.root, exclude=bundle.all_ids,
                                   negatives=args.negative_depth,
    ...
    if recorder.best_epoch is None:
        print("No epoch beat the cached baseline -- keep the shipped checkpoint.\n"
              "Remember this project's noise floor: a Recall difference under "
              "0.008 is not evidence of anything.", flush=True)
