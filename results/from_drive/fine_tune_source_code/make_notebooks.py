...
MODELS = [
    ...
    },
    {
        ...
        "train_top_layers": 8,
        "max_gpus": 1,
        "colab_file": "colab_finetune_aiteamvn.ipynb",
        "colab_batch_size": 8,        # 2x Kaggle -- tỷ lệ bảo thủ so với 2.67x VRAM
                                       # thật (T4 15GB -> A100 40GB); chưa đo thực tế
                                       # trên GPU thật, xem README phần Colab trước khi
                                       # tăng thêm
        "colab_accum": 2,             # giữ nguyên effective batch = 16
        "colab_eval_batch_size": 48,
        "why": (
    ...
    },
    {
        ...
        "train_top_layers": 0,
        "max_gpus": 0,
        "colab_file": "colab_finetune_jina.ipynb",
        "colab_batch_size": 8,        # 2x Kaggle -- tỷ lệ bảo thủ, cross-encoder forward
                                       # nặng hơn bi-encoder/batch nên không tăng thêm;
                                       # chưa đo thực tế trên GPU thật
        "colab_accum": 2,             # giữ nguyên effective batch = 16
        "colab_eval_batch_size": 32,
        "why": (
...
def splitlines(source):
    """nbformat wants every line to keep its newline; some readers join the
    list with "" and a cell written without them collapses into one line."""
    return source.rstrip("\n").splitlines(keepends=True) or [""]


def code(source):
    return {"cell_type": "code", "execution_count": None, "metadata": {},
            "outputs": [], "source": splitlines(source)}


def build(model, target="kaggle"):
...
Notebook được cắt thành từng cell nhỏ, **mỗi epoch một cell**. `WORK` nằm
thẳng trên Google Drive (`/content/drive/MyDrive/fine_tune_work/{tag}/`), nên
kết quả **tự động persist** sau mỗi epoch — không cần tải zip thủ công như
Kaggle. Nếu runtime Colab bị ngắt, cứ **chạy lại từ cell 1**: notebook tự đọc
`history.json` cũ, bỏ qua epoch đã ghi và nạp lại `best_state.pt`, không tốn
lại GPU của phần đã xong. Muốn làm lại sạch thì đặt `RESUME = False`.

**Runtime cần chọn: GPU (A100), high-RAM nếu có.**"""
    else:
        run_mode = f"""\
## Cách chạy để không mất kết quả
...
models/ + code `fine_tune/`) lên đúng đường dẫn này trong Drive, giữ nguyên
cấu trúc con. `WORK` (checkpoint + log) ghi ra một thư mục Drive khác
...
LR              = {model["lr"]}
BATCH_SIZE      = {model["colab_batch_size"]}        # A100 nhiều VRAM hơn T4 -- xem README phần Colab
ACCUM           = {model["colab_accum"]}         # cùng effective batch = BATCH_SIZE*ACCUM như bản Kaggle
NEGATIVES       = 7        # hard negative mỗi truy vấn mỗi bước
NEGATIVE_DEPTH  = 48       # lấy negative sâu bao nhiêu trong pool lexical đã cache
MAX_LENGTH      = 512
EVAL_BATCH_SIZE = {model["colab_eval_batch_size"]}
PRECISION       = "auto"   # auto | fp16 | bf16 | fp32 -- xem cell 4 (A100 có bf16 gốc)
MAX_GPUS        = 1        # Colab thường cấp 1 GPU (A100) -- không cần DataParallel
...
assert torch.cuda.is_available(), "Runtime > Change runtime type > GPU (A100) rồi chạy lại"
...
`REF` là dataset `fine_tune` — zip nguyên folder `ref/` (results/ cache +
DSC2026-LegalIR-main/ corpus + models/ + code `fine_tune/`), ~600 MB, gần
như không bao giờ đổi.
`CODE` mặc định lùi về `REF/fine_tune` (code nằm sẵn trong cùng dataset
`fine_tune`). Vẫn có thể tách code ra dataset `fine-tune-code` riêng nếu
muốn sửa code mà không upload lại 600 MB — không bắt buộc."""))
        cells.append(code(f'''\
import os, sys, json, time
from pathlib import Path
...
REF  = Path("/kaggle/input/fine_tune")           # dataset nhtclone/fine_tune -- zip của cả folder ref/
...
'''))
    ...
    cells.append(md("## Cell 3 — Tập huấn luyện\n\n"
                    "Toàn bộ truy vấn có nhãn **đã có sẵn cache retrieval** "
                    "(~1.650 truy vấn, index 0-1149 và 1250-1749) — bao gồm cả "
                    "600 truy vấn của block LOBO dùng để đánh giá bên dưới. "
                    "Hard negative lấy thẳng từ pool lexical đã cache — không "
                    "chạy lại retrieval.\n\n"
                    "⚠️ Vì tập train giờ **giao hoàn toàn** với tập đánh giá, "
                    "Recall/F2 in ra sau mỗi epoch không còn là ước lượng "
                    "generalization — model đã thấy chính các truy vấn đó lúc "
                    "train. Đây là đánh đổi có chủ đích để tận dụng hết dữ liệu "
                    "nhãn cho lần fine-tune cuối, không phải một lỗi.\n\n"
                    "`RunRecorder` chọn `best_state.pt` theo **epoch tốt nhất "
                    "của chính lần chạy này** trên holdout 600 truy vấn: epoch "
                    "đầu luôn được lưu, epoch sau chỉ ghi đè khi validate tăng. "
                    "Việc có vượt baseline cache hay không là một câu hỏi khác "
                    "và được trả lời riêng ở cột `vượt baseline` trong bảng "
                    "tổng kết — nó không còn quyết định chuyện lưu hay không, "
                    "vì baseline của kênh `jina` đến từ một checkpoint không "
                    "nằm trong gói ref."))
    cells.append(code(f'''\
...
Tải trọng số từ HuggingFace. Nếu đang resume, trọng số tốt nhất của lần chạy
trước sẽ được nạp đè.

Colab thường cấp **1 GPU** (A100), nên `nn.DataParallel` không có gì để chia
— `wrap_parallel` chỉ là no-op ở đây, không cần lo. Nếu bạn thuê runtime nhiều
GPU thì code vẫn tự bọc DataParallel đúng như trên Kaggle.

### Kiểm tra độ chính xác số học

A100 (compute capability 8.0) **có bf16 gốc**, dải số rộng hơn hẳn fp16
(~1e38 so với 65504) nên autocast mặc định chọn bf16 — hiếm khi tràn số như
fp16 trên T4. Cell này vẫn chạy thử **một batch** trước khi vào epoch thật:
nếu dtype được chọn cho kết quả không hữu hạn, nó tự rơi về fp32 (chậm hơn
~2 lần nhưng đổi lấy một epoch không NaN thì quá rẻ). Ép tay bằng
`PRECISION = "fp32"` ở cell 1 nếu cần."""
    else:
        cell4_md = """\
## Cell 4 — Nạp model
...
model = tc.wrap_parallel(model, n_gpus)     # >1 GPU -> mỗi batch chia đều, no-op nếu chỉ 1 GPU
'''))
...
'''))
    ...
    for epoch in (1, 2, 3):
        if colab:
            epoch_note = "Kết quả tự ghi thẳng vào Drive, không cần tải gì." if epoch < 3 else "Epoch cuối."
        else:
            epoch_note = (f"Chạy xong nhớ tải `{tag}_results.zip` về nếu đang chạy "
                          "tương tác." if epoch < 3 else "Epoch cuối.")
...
def main():
    for model in MODELS:
        ...
        print(f"wrote {path}")
    for model in MODELS:
        if "colab_file" not in model:
            continue
        path = HERE / model["colab_file"]
        path.write_text(json.dumps(build(model, target="colab"), ensure_ascii=False,
