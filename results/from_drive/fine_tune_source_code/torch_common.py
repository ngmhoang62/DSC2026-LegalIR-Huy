...
def patch_hub_code_compat():
    """Make transformers 5.x satisfy the imports jina's hub code was written for.

    `trust_remote_code=True` executes modeling code the model author uploaded in
    2024 against whatever transformers Kaggle ships today.  transformers 5
    dropped `create_position_ids_from_input_ids` from the xlm_roberta module
    during the modular refactor, so jina's embedding.py fails at import with

        ImportError: cannot import name 'create_position_ids_from_input_ids'

    The helper's body never changed, so putting it back is exact, not a
    workaround: the padding-aware position ids below are the transformers 4
    implementation verbatim.  The other two names are re-exports that may or may
    not still be re-exported; alias them from where they live now if needed.
    Nothing is touched when the symbol is already present, so this is a no-op on
    an older transformers.
    """
    added = []
    xr = importlib.import_module("transformers.models.xlm_roberta.modeling_xlm_roberta")
    if not hasattr(xr, "create_position_ids_from_input_ids"):
        def create_position_ids_from_input_ids(input_ids, padding_idx,
                                               past_key_values_length=0):
            mask = input_ids.ne(padding_idx).int()
            steps = (torch.cumsum(mask, dim=1).type_as(mask) + past_key_values_length) * mask
            return steps.long() + padding_idx

        xr.create_position_ids_from_input_ids = create_position_ids_from_input_ids
        added.append("xlm_roberta.create_position_ids_from_input_ids")
    ...
    for name in ("BaseModelOutputWithPoolingAndCrossAttentions",
                 "BertForPreTrainingOutput"):
        if not hasattr(bert, name) and hasattr(outputs, name):
            setattr(bert, name, getattr(outputs, name))
            added.append(f"bert.{name}")
    if not hasattr(xr, "XLMRobertaLMHead"):
        roberta = importlib.import_module("transformers.models.roberta.modeling_roberta")
        if hasattr(roberta, "RobertaLMHead"):
            xr.XLMRobertaLMHead = roberta.RobertaLMHead
            added.append("xlm_roberta.XLMRobertaLMHead")
    print(f"transformers compat shims: {', '.join(added) if added else 'none needed'}",
          flush=True)
    return added


def load_fp32(auto_class, source, **kwargs):
    """from_pretrained, forced to fp32 whatever dtype the checkpoint carries.

    jina's config.json says `torch_dtype: bfloat16` -- the inference form of the
    weights.  transformers 4 ignored that and gave fp32; transformers 5 honours
    it, so the same call now hands back a bf16 model.  Training that on a T4
    (compute 7.5, no native bf16) is both slow and unstable, and the optimizer
    would keep its master weights at the checkpoint's precision.  Ask for fp32
    explicitly, under both the new spelling and the old one.
    """
    try:
        model = auto_class.from_pretrained(source, dtype=torch.float32, **kwargs)
    except TypeError:
        model = auto_class.from_pretrained(source, torch_dtype=torch.float32, **kwargs)
    dtype = next(model.parameters()).dtype
    if dtype != torch.float32:
        model = model.float()
        print(f"cast {dtype} -> float32 after loading", flush=True)
    return model
...
# --------------------------------------------------------------------------
# Generative yes/no cross-encoder (Qwen3-Reranker family) scoring
# --------------------------------------------------------------------------

YESNO_SYSTEM_PROMPT = ('Judge whether the Document meets the requirements based on the '
                       'Query and the Instruct provided. Note that the answer can only '
                       'be "yes" or "no".')


def causal_yesno_logits(model, tokenizer, questions, passages, max_length, device,
                        instruction, system_prompt=YESNO_SYSTEM_PROMPT):
    """Differentiable relevance logit for a generative yes/no reranker.

    Qwen3-Reranker-4B and infgrad/Prism-Qwen3.5-Reranker-2B are causal LMs, not
    sequence-classification heads: relevance is read off the next-token
    distribution after a chat prompt ending right before the answer, as
    P(yes)/P(no) at that one position -- both model cards document the score
    as sigmoid(logit(yes) - logit(no)), so returning the raw difference here
    makes this a drop-in for cross_encoder_logits (same RankNet/listwise loss,
    same reduce_max document pooling).

    Left padding is required: with right padding the last real token would
    not sit at position -1 for every row in the batch, and the read-out below
    would silently score the wrong (pad) position for the shorter sequences.
    """
    ...
    token_true_id = tokenizer.encode("yes", add_special_tokens=False)[-1]
    token_false_id = tokenizer.encode("no", add_special_tokens=False)[-1]
    budget = max(1, max_length - len(prefix_ids) - len(suffix_ids))

    contents = [f"<Instruct>: {instruction}\n<Query>: {q}\n<Document>: {p}"
               for q, p in zip(questions, passages)]
    body = tokenizer(contents, add_special_tokens=False, truncation=True, max_length=budget)
    input_ids = [prefix_ids + ids + suffix_ids for ids in body["input_ids"]]
    batch = tokenizer.pad({"input_ids": input_ids}, padding=True, return_tensors="pt")
    batch = {k: v.to(device, non_blocking=True) for k, v in batch.items()}
    logits = model(**batch, return_dict=True).logits[:, -1, :]
    return (logits[:, token_true_id] - logits[:, token_false_id]).float()


@torch.inference_mode()
def score_causal_yesno(model, tokenizer, questions, pool, documents, ids,
                       device, instruction, system_prompt=YESNO_SYSTEM_PROMPT,
                       max_length=bc.MAX_LENGTH, batch_size=8,
                       passages_per_doc=bc.PASSAGES_PER_DOC,
                       label="qwen3_reranker"):
    """Score every (query, candidate) pair with the generative yes/no head.

    No autocast here on purpose: the caller loads these models frozen in fp16
    with LoRA adapters upcast to fp32 (see finetune_qwen3_reranker.py), so the
    dtype is already fixed at load time rather than chosen per forward pass
    the way the fp32-loaded jina/bi-encoder channels are.
    """
    model.eval()
    scores = {}
    started = time.perf_counter()
    ...
    for q in bar:
        ...
        for start in range(0, len(passages), batch_size):
            chunk = passages[start:start + batch_size]
            values.extend(causal_yesno_logits(
                model, tokenizer, [question] * len(chunk), chunk,
                max_length, device, instruction,
                system_prompt=system_prompt).cpu().tolist())
        scores[q], bad = reduce_max(owners, values, pool[q])
        bad_total += bad
        seen_passages += len(passages)
        bar.set_postfix(passages=seen_passages, bad=bad_total,
