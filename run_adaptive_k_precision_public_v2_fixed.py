...
def main():
    ...
    three_view_obj = load_pickle(
        root / "results/burst_gpu_threeview/gpu_scores.checkpoint.pkl"
    )
    if not isinstance(three_view_obj, dict) or not isinstance(
        three_view_obj.get("scores"), dict
    ):
        raise RuntimeError(
            "Unexpected gpu_scores.checkpoint.pkl schema: expected top-level 'scores'"
        )
    three_view_pub = three_view_obj["scores"]
    ...
    for q in public_ids:
        ...
        groups_q = [str(d) for d in pub_groups[q]]
        smap = {
            d: float(v)
            for d, v in zip(groups_q, scores)
        }
        # Exact production ordering contract: np.argsort(-scores), no custom
        # doc-id tie-break.  This must reproduce CANDIDATE_D1 exactly.
        order_idx = np.argsort(-scores)
        order = [groups_q[i] for i in order_idx]
        d1_ordered[q] = order
        d1_scoremaps[q] = smap
