...
def main():
    ...
    if exclusions_path.exists():
        try:
            ...
            # This source is OPTIONAL. Core corpus/D1 collision audit does not
            # depend on it; never abort the scientific report because a sibling
            # workspace file is unreadable on Windows.
            exclusion_summary = {
                "path": str(exclusions_path),
                "available": False,
                "status": "OPTIONAL_SOURCE_UNREADABLE",
    ...
    if exclusion_summary.get("available"):
        print(
            "Sibling exclusions: "
        ...
        print(
            "Sibling exclusions: unavailable "
            f"({exclusion_summary.get('status', 'UNKNOWN')})"
        )
    print(
