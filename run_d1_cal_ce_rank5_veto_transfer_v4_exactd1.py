...
import argparse, gc, hashlib, importlib.util, json, re, sys, time
...
def load_exact_d1_cal_top5_label_only(root: Path):
    """Load exact D1 CAL OOF Top5 without materializing co-located gold labels.

    Authoritative source:
      results/gemini/huy_d1_legal_section_evidence_v1/S0_S1_CAL_PREDICTIONS.jsonl

    That historical artifact also contains gold_docs, so we intentionally DO NOT
    json.loads() the whole line.  We whitelist-extract only qid and s0_top5.
    """
    path = (
        root
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"Authoritative exact-D1 CAL artifact missing: {path}"
        )
    ...
    with path.open("r", encoding="utf-8") as f:
        for lineno, line in enumerate(f, 1):
            ...
            if qm is None or tm is None:
                raise RuntimeError(
                    f"Could not whitelist-extract qid/s0_top5 at {path}:{lineno}"
                )
            q = str(qm.group(1))
            docs = [str(d) for d in json.loads(tm.group(1))]
            if len(docs) != 5 or len(set(docs)) != 5:
                raise RuntimeError(
                    f"Invalid exact D1 s0_top5 qid={q}: {docs}"
                )
            if q in base:
                raise RuntimeError(f"Duplicate qid in exact D1 artifact: {q}")
            ids.append(q)
            base[q] = docs

    if len(ids) != 600 or len(base) != 600:
        raise RuntimeError(
            f"Expected exact D1 CAL population=600, got ids={len(ids)} unique={len(base)}"
        )

    fp = hashlib.sha256(
        json.dumps(
            [(q, base[q]) for q in ids],
            ensure_ascii=False,
            separators=(",", ":"),
        ).encode("utf-8")
    ).hexdigest()
    return ids, base, path, fp


@torch.inference_mode()
def score_cal_top5(m, root, sibling, world, model_path, pair_microbatch, out):
    # Exact local D1 OOF baseline, field-whitelist loaded from the
    # authoritative S0/S1 CAL artifact.
    ids, base, baseline_path, baseline_fp = load_exact_d1_cal_top5_label_only(root)
    print(
        f"  exact D1 source={baseline_path}\\n"
        f"  exact D1 population={len(ids)} fingerprint={baseline_fp}",
        flush=True,
    )
    ...
    score_dir=out/"cal_exact_d1_top5_ce_scores_v1"
    score_dir.mkdir(parents=True,exist_ok=True)
    rows={}
    no_evidence_abstentions={}
    ...
    try:
        for i,q in enumerate(eligible,1):
            ...
            # Conservative semantic-veto contract:
            # if ANY D1 Top5 parent is absent from the frozen Evidence inventory,
            # do not invent a different rendering and do not partially score.
            # Abstain for the whole query and preserve exact D1 Top5.
            missing_docs = [
                d for d in top5
            ...
            if missing_docs:
                no_evidence_abstentions[q] = missing_docs
                if i % 50 == 0 or i == len(eligible):
                    print(
                        f"  CE scan {i}/{len(eligible)} "
                        f"scored={len(rows)} abstain_no_evidence={len(no_evidence_abstentions)} "
            ...
            p=score_dir/f"{q}.json"
            sig=hashlib.sha256(
                json.dumps([sha256(model_path),q,top5,"exact-d1-oof-top5-ce-v1"],sort_keys=True).encode()
            ).hexdigest()
    ...
    return ids,base,eligible,abstain,rows,no_evidence_abstentions
...
def main():
    ...
    out=root/"results/manual/huy_d1_cal_ce_rank5_veto_transfer_v2_exactd1"
    ...
    ids,base,eligible,abstain,rows,no_evidence_abstentions=score_cal_top5(
        m,root,sibling,world,model_path,args.pair_microbatch,out
    )
    scorable_ids = [q for q in eligible if q in rows]
    print(
        f"  CE scorable={len(scorable_ids)} | "
        f"no-evidence abstentions={len(no_evidence_abstentions)} | "
        f"outside-V2 abstentions={len(abstain)}",
        flush=True,
    )
    if no_evidence_abstentions:
        print(
            "  no-evidence sample: "
            + json.dumps(
                list(no_evidence_abstentions.items())[:10],
                ensure_ascii=False,
            ),
            flush=True,
        )
    ...
    seal={
        ...
        "d1_baseline_contract":{
            "source":"results/gemini/huy_d1_legal_section_evidence_v1/S0_S1_CAL_PREDICTIONS.jsonl",
            "field_used":"s0_top5 only",
            "expected_recall_at_5":EXPECTED_D1_R,
            "expected_precision_macro_at_5":EXPECTED_D1_P,
        },
        "fold0_dev_stats":fold_stats,
        "arms":arms,
        "eligible_cal_v2":len(eligible),
        "ce_scorable_cal":len(scorable_ids),
        "cal_outside_v2_abstentions":abstain,
        "cal_no_evidence_abstentions":no_evidence_abstentions,
    ...
    if abs(baseline["recall"]-EXPECTED_D1_R)>1e-12:
        raise RuntimeError(f"AUTHORITATIVE exact D1 recall parity failed: {baseline}")
    if abs(baseline["precision_macro"]-EXPECTED_D1_P)>5e-10:
        raise RuntimeError(f"AUTHORITATIVE exact D1 precision parity failed: {baseline}")
