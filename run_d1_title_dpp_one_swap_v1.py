...
Requires CUDA only for one-time title embedding. Cached embeddings are reused.
"""
...
def title_embedding_cache(
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is required to create title embeddings on first run."
        )
    ...
    model = (
        AutoModel.from_pretrained(model_path, dtype=torch.float16)
        .eval()
        .to("cuda")
    )
    ...
    # encode_cls already L2-normalizes; re-normalize defensively.
