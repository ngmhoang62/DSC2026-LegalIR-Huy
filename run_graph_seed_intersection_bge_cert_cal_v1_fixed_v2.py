...
def main():
    ...
    # Layout autodiscovery.
    #
    # Current repo layout may keep helper scripts directly under repo root
    # (D:/Study/DSC2026/sota) instead of the historical parent directory.
    # Prefer the current repo root, then fall back to the old layout.
    base_script_candidates = [
        root / "run_noncal_trainable_ce_boundary_v3_fixed.py",
        root.parent / "run_noncal_trainable_ce_boundary_v3_fixed.py",
    ]
    base_script = next(
        (p for p in base_script_candidates if p.is_file()),
        None,
    )
    if base_script is None:
        raise FileNotFoundError(
            "Frozen CE world loader not found. Checked:\n  "
            + "\n  ".join(str(p) for p in base_script_candidates)
        )

    # The historical CE codebase is usually a sibling checkout named LegalIR.
    # Also support it being moved inside the repo root.
    sibling_candidates = [
        root / "LegalIR",
        root.parent / "LegalIR",
    ]
    sibling = next(
        (p for p in sibling_candidates if p.is_dir()),
        None,
    )
    if sibling is None:
        raise FileNotFoundError(
            "LegalIR helper checkout not found. Checked:\n  "
            + "\n  ".join(str(p) for p in sibling_candidates)
        )

    print(f"  CE world loader: {base_script}", flush=True)
    print(f"  LegalIR helper:   {sibling}", flush=True)

    # Resolve the CAL loader BEFORE the CE helper prepends the sibling LegalIR
    # checkout to sys.path. Otherwise a sibling `src` package can shadow this
    # repo's `src` package late in the run.
    if str(root) in sys.path:
        sys.path.remove(str(root))
    sys.path.insert(0, str(root))
    try:
        from src.gemini.huy_vnlegal_rank_ablation_v1.evaluate_ablation_cal import (
            load_cal_inputs as load_cal_inputs_for_blocks,
        )
    except ModuleNotFoundError as exc:
        cal_loader_path = (
            root
            / "src/gemini/huy_vnlegal_rank_ablation_v1/"
              "evaluate_ablation_cal.py"
        )
        raise ModuleNotFoundError(
            "Could not load CAL block metadata from the current repo before "
            "initializing the sibling LegalIR CE stack. Expected file:\n  "
            f"{cal_loader_path}\nOriginal error: {exc}"
        ) from exc
    print(
        "  CAL block loader: "
        f"{root / 'src/gemini/huy_vnlegal_rank_ablation_v1/evaluate_ablation_cal.py'}",
        flush=True,
    )
    ...
    # Obtain authoritative CAL blocks only now, after seal. The callable was
    # imported before the sibling CE stack modified sys.path, preventing the
    # sibling `src` package from shadowing this repo's `src`.
    (
        _queries,
        blocks,
        all_ids2,
