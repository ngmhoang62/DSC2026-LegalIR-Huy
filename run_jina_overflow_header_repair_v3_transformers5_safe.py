...
def patch_transformers_v5_for_jina() -> None:
    """Restore the helper expected by Jina's bundled remote modeling code.

    transformers v5 removed create_position_ids_from_input_ids from the
    XLM-Roberta module, while the frozen Jina remote code still imports it.
    This is the same compatibility shim already used elsewhere in this repo
    (score_public_models.py / legal-section experiments).
    """
    ...
    def helper(input_ids, padding_idx, past_key_values_length=0):
        ...
        positions = (
            torch.cumsum(mask, dim=1) + past_key_values_length
        ) * mask
...
def patch_jina(root, world, batch_size=16):
    ...
    patch_transformers_v5_for_jina()
