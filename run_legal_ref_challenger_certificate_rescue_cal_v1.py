...
- DO NOT gate on defender absolute weakness.

- Scan generated additions IN THEIR SEALED ORDER and choose the FIRST candidate
  with frozen Evidence coverage satisfying BOTH:
      CE(candidate) > CE(defender)
...
def main():
    ...
    out = (
        root
        / "results/manual/"
        "huy_legal_ref_challenger_certificate_rescue_cal_v1"
    )
    ...
    print("[3/7] Preparing ALL frozen legal-ref trigger queries (NO defender weakness gate)...")
    requests = {}
    prelim = {}
    ...
    print(
        ...
        f"queries sent to challenger CE={len(requests)}",
        flush=True,
    )
    ...
    for q in ids:
        ...
        for idx, (d, detail) in enumerate(
            ...
            rel = score - med
            beats_defender = score > row["defender_score"]
            abs_cert = rel >= REL_L0
    ...
    seal = {
        "schema": "manual.legal_ref_challenger_certificate_rescue_cal_v1.actions",
    ...
    verdict = (
        ...
        else "KILL_LEGAL_REF_CHALLENGER_CERT_POLICY"
    )

    report = {
        "schema": "manual.legal_ref_challenger_certificate_rescue_cal_v1.report",
        "verdict": verdict,
