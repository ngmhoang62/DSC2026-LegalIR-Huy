...
def get_cal_ids_label_free(root: Path) -> Tuple[List[str], Dict[str, str]]:
    """Recover CAL600 ids/questions without importing optional Gemini packages."""
    baseline_path = (
    ...
    with transfer_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            q = str(row["qid"])
            if q in baseline:
                questions_all[q] = str(row["question"])

    if set(questions_all) != set(ids):
        missing = sorted(set(ids) - set(questions_all), key=int)
        raise RuntimeError(
            f"V2 bundle does not cover all CAL questions; missing={missing[:20]}"
        )

    return ids, {q: questions_all[q] for q in ids}
