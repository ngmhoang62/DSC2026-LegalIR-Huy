...
Leakage contract
----------------
CAL600 qids are obtained LABEL-FREE first; their V2-evaluable overlap is excluded from:
  - every training fold
  - every OOF gate metric
  - final training

Only Strict-V2 qids outside CAL are used for fitting / validation. CAL-only queries outside V2 automatically abstain.
...
def get_cal_ids_label_free(root: Path) -> Tuple[List[str], Dict[str, str]]:
    """Recover CAL600 ids plus V2-available question texts, label-free.

    BASELINE_LOBO_PREDICTIONS defines all 600 CAL ids.  The V2 transfer bundle
    contains question text only for the V2-evaluable subset.  CAL queries absent
    from V2 are intentionally left without a transfer representation and will
    be automatic abstentions at CAL inference.
    """
    baseline_path = (
        root / "results/sol_high_rl/BASELINE_LOBO_PREDICTIONS.json"
    )
    transfer_path = (
        root
    ...
    ids = [str(q) for q in baseline.keys()]
    if len(ids) != 600 or len(set(ids)) != 600:
        raise RuntimeError(
            f"Expected exactly 600 unique CAL ids from D1 artifact, got {len(ids)}"
        )

    cal_set = set(ids)
    questions_v2: Dict[str, str] = {}
    with transfer_path.open("r", encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            row = json.loads(line)
            q = str(row["qid"])
            if q in cal_set:
                questions_v2[q] = str(row["question"])

    if not questions_v2:
        raise RuntimeError("No CAL/V2 question overlap recovered")
    if not set(questions_v2).issubset(cal_set):
        raise RuntimeError("Recovered non-CAL questions unexpectedly")

    return ids, questions_v2
...
def load_noncal_world(
    ...
    v2_set = set(all_ids)
    cal_overlap = sorted(v2_set & cal_ids, key=int)
    cal_outside_v2 = sorted(cal_ids - v2_set, key=int)
    noncal = [q for q in all_ids if q not in cal_ids]

    if len(all_ids) != 6991:
        raise RuntimeError(f"V2 population mismatch: {len(all_ids)}")
    if len(noncal) + len(cal_overlap) != len(all_ids):
        raise RuntimeError("V2 population partition mismatch")
    if len(cal_overlap) + len(cal_outside_v2) != len(cal_ids):
        raise RuntimeError("CAL population partition mismatch")
    ...
    return {
        ...
        "lal": lal,
        "noncal": noncal,
        "cal_overlap": cal_overlap,
        "cal_outside_v2": cal_outside_v2,
        "render": render,
        "source_db": source_db,
...
def run_oof(
    *,
    ...
    report = {
        ...
        "population": {
            "all_v2": len(world["questions"]),
            "cal_v2_overlap_excluded": len(world["cal_overlap"]),
            "cal_outside_v2": len(world["cal_outside_v2"]),
            "noncal": len(noncal),
        },
        "mechanism": {
...
def load_cal_frontier_label_free(root: Path):
    """Reconstruct exact CAL frontier without optional Gemini modules or CAL gold."""
    ids, questions = get_cal_ids_label_free(root)
    ...
    e5_path = (
        root
        / "results/manual/huy_cal600_adapted_e5_full_corpus_v1/"
        "CAL600_ADAPTED_E5_FULL_CORPUS_TOP150.json"
    )
    ai_path = (
        root / "results/sol_high_rl/AITEAM_FT_FULL_CORPUS_TOP50.json"
    )
    extended_scores_path = (
        root / "results/corpus_index/holdout_extended_scores_cap32.pkl"
    )

    for p in (baseline_path, e5_path, ai_path, extended_scores_path):
        if not p.is_file():
            raise FileNotFoundError(p)

    base = {
        str(q): [str(d) for d in row]
        for q, row in json.loads(
    ...
    extended_scores = pickle.loads(extended_scores_path.read_bytes())
    if (
        not isinstance(extended_scores, dict)
        or "jina" not in extended_scores
        or not isinstance(extended_scores["jina"], dict)
    ):
        raise RuntimeError(
            "Unexpected holdout_extended_scores_cap32.pkl schema"
        )

    pool = {}
    for q in ids:
        row = extended_scores["jina"].get(q)
        if not isinstance(row, dict) or not row:
            raise RuntimeError(f"Missing D1 cap32 candidate scores qid={q}")
        pool[q] = [str(d) for d in row.keys()]

    expected = set(ids)
    if not (
        set(base) == set(e5) == set(ai) == set(pool) == expected
    ):
        raise RuntimeError("CAL frontier population mismatch")

    for q in ids:
        if len(base[q]) < 5:
            raise RuntimeError(f"D1 prediction shorter than 5 qid={q}")
        if not set(base[q][:5]).issubset(set(pool[q])):
            raise RuntimeError(
                f"D1 Top5 not contained in reconstructed cap32 pool qid={q}"
            )

    # BASELINE_LOBO_PREDICTIONS was materialized block-by-block A,B,C,D.
    blocks = {
        "A": ids[:100],
        "B": ids[100:200],
        "C": ids[200:300],
    ...
    if [len(blocks[k]) for k in ("A", "B", "C", "D")] != [100, 100, 100, 300]:
        raise RuntimeError("CAL block reconstruction failed")
...
def final_fit_and_cal(
    *,
    ...
    # Only CAL queries represented in the V2-evaluable world are eligible for
    # transfer intervention.  Any CAL-only/non-V2 query is an automatic abstain
    # and keeps the exact D1 Top5.
    eligible_ids = [
        q for q in ids
        if q in world["questions"] and q in cal_questions
    ]
    abstained_no_v2 = [q for q in ids if q not in set(eligible_ids)]

    if set(eligible_ids) != set(world["cal_overlap"]):
        raise RuntimeError(
            "CAL/V2 eligible population disagrees with sanitized world"
        )
    if set(abstained_no_v2) != set(world["cal_outside_v2"]):
        raise RuntimeError(
            "CAL outside-V2 abstention population disagrees with sanitized world"
        )
    ...
    partial_modified, actions = score_frontier(
        sibling=sibling,
        render=world["render"],
        ids=eligible_ids,
        base=base,
    ...
    modified = {q: list(base[q][:5]) for q in ids}
    modified.update(partial_modified)
    ...
    action_doc = {
        ...
        "cal_total_queries": len(ids),
        "cal_v2_supported_queries": len(eligible_ids),
        "cal_outside_v2_abstentions": abstained_no_v2,
        "candidate_prior": (
    ...
    if not cal_gold_path.is_file():
        raise FileNotFoundError(cal_gold_path)
    raw_gold = json.loads(cal_gold_path.read_text(encoding="utf-8"))
    gold = {
        q: {str(d) for d in raw_gold[q]["answer"]}
        for q in ids
    }
...
def main():
    ...
    print(
        f"  all V2={len(world['questions'])} | "
        f"CAL∩V2 excluded={len(world['cal_overlap'])} | "
        f"CAL outside V2={len(world['cal_outside_v2'])} | "
        f"nonCAL={len(world['noncal'])}",
        flush=True,
    )
    if world["cal_outside_v2"]:
        print(
            "  transfer abstention for CAL outside V2: "
            + ",".join(world["cal_outside_v2"]),
            flush=True,
        )

    # Strong question-identity assertion on the transfer-eligible CAL subset.
    for q in world["cal_overlap"]:
        if world["questions"][q] != cal_questions[q]:
            raise RuntimeError(
