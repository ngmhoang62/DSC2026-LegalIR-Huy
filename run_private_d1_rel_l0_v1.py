...
def target_base_top20(root: Path, corpus_paths, train, doc_ids, retrieval, questions, ids, cache_path: Path):
    ...
    large_model = load_pickle(root / "results/burst_large_ltr/best_model.pkl")["model"]
    legal_model = load_pickle(root / "results/burst_legal_features/validation_model.pkl")["model"]
    pair_saved = load_pickle(root / "results/burst_empirical_pairwise/model.pkl")
...
def main():
    ...
    retrieval = target_retrieval(
        root, root / "benchmarks/legalir_full_fts.sqlite", train, doc_ids,
        questions, ids, cache / "private_retrieval.pkl", args.workers
    )
    base = target_base_top20(
