#!/usr/bin/env python
"""Private D1 + frozen REL_L0 materializer.

Rebuilds production D1 on private-official.json, then applies the already
validated drop-only policy:
  REL = CE(rank5) - median(CE(rank1..4))
  drop rank5 iff REL < -3.0393552780151367.

All private caches are isolated under results/manual/huy_private_d1_rel_l0_v1.
The script first requires 1000/1000 cached-public D1 downstream parity.
"""
...
def target_base_top20(root: Path, corpus_paths, train, doc_ids, retrieval, questions, ids, cache_path: Path):
    ...
    from tune_burst_supervised_profile_bm25 import build_profiles
    from tune_burst_multistage_posterior import weighted_rrf
    from tune_burst_pairwise import blend_rankings
    from tune_burst_score_ltr import score_features
    from tune_burst_supervised_profile_bm25 import profile_rank
    ...
    print(f"  base-top20 cache {len(rankings)}/{len(ids)} missing={len(missing)}", flush=True)

    print("  resolving historical CPU ranker artifacts...", flush=True)
    large_path, large_saved = resolve_historical_artifact(
        root=root,
        rel_path="results/burst_large_ltr/best_model.pkl",
        validator=_validate_large_ltr,
    )
    legal_path, legal_saved = resolve_historical_artifact(
        root=root,
        rel_path="results/burst_legal_features/validation_model.pkl",
        validator=_validate_legal_model,
    )
    pair_path, pair_saved = resolve_historical_artifact(
        root=root,
        rel_path="results/burst_empirical_pairwise/model.pkl",
        validator=_validate_pair_model,
    )
    artifact_provenance = {
        "large_ltr": {
            "path": str(large_path),
            "sha256": sha256(large_path),
            "version": large_saved.get("version"),
        },
        "legal_features": {
            "path": str(legal_path),
            "sha256": sha256(legal_path),
            "feature_version": legal_saved.get("feature_version"),
        },
        "empirical_pairwise": {
            "path": str(pair_path),
            "sha256": sha256(pair_path),
            "feature_version": pair_saved.get("feature_version"),
        },
    }
    ...
    print("  normalizing corpus for legal feature ranker...", flush=True)
    normalized = {}
    ...
    legal_model = legal_saved["model"]
    public_queries = {q: (questions[q], set()) for q in ids}
    pair_features = features_for(retrieval, public_queries, ids, frequency)
    started = time.perf_counter()
    for i, q in enumerate(missing, 1):
        lists = retrieval[q]
        c1, x1 = score_features(lists)
        large = [c1[j] for j in np.argsort(-large_model.predict(x1))[:100]]
        c2, x2 = enhanced_features(lists, questions[q], normalized, lexical_depth=20)
        legal = [c2[j] for j in np.argsort(-legal_model.decision_function(x2))[:100]]
        robust = blend_rankings({q: large}, {q: legal[:10]}, .275, 40)[q]
        pair = pairwise_rank(pair_saved["model"], pair_saved["scaler"], {q: pair_features[q]}, [q])[q]
        profile = profile_rank(questions[q], profile_model, 2, 1.2, .75, .3)
        graph = graph_rerank(robust, graph_frequency, graph_adjacency, 3, .5, "conditional", 3, .4, 0)
        final = weighted_rrf([{q: robust}, {q: pair}, {q: profile}, {q: graph}], (.40, .30, .15, .15), 0)[q]
        ...
        rankings[q] = top20
        if i % 25 == 0 or i == len(missing):
            save_pickle(cache_path, {"qids": ids, "question_fingerprint": fp, "rankings": rankings})
            print(f"    base-top20 {i}/{len(missing)} ({time.perf_counter()-started:.1f}s)", flush=True)
    return rankings, artifact_provenance
...
def main():
    ...
    out = root / "results/manual/huy_private_d1_rel_l0_v1"
    cache = out / "cache"
    ...
    print("[4/12] Fresh isolated private sparse retrieval + CPU base Top20...", flush=True)
    ...
    retrieval = target_retrieval(
        ...
        questions, ids, cache / "private_retrieval.pkl", args.workers
    )
    base, cpu_base_artifacts = target_base_top20(
        root, corpus_paths, train, doc_ids, retrieval, questions, ids,
        cache / "private_base_top20.pkl"
    )

    print("[5/12] Building private expanded/corpus candidates + rerank views...", flush=True)
    ...
    d1_json, d1_zip = out / "D1_PRIVATE.json", out / "D1_PRIVATE.zip"
    ...
    zip_path = rel_dir / "submission_REL_L0_PRIVATE.zip"
    dump(action_path, {
        "schema": "manual.private_d1_rel_l0_v1.actions",
        "status": "SEALED_LABEL_FREE_PRIVATE", "threshold": REL_L0,
    ...
    report = {
        "schema": "manual.private_d1_rel_l0_v1.report",
        ...
        "historical_generator_caveat": {
            ...
            "private_reconstruction": (
                ...
                "all private caches isolated from public caches"
            ),
        },
        "d1": {
