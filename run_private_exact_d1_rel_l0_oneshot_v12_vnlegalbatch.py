...
def score_vnlegal_channel(
    ...
    """
    Cache-compatible vnlegal-lal scorer.

    Historical/private D1 contract is unchanged:
      - same darklethelong/vnlegal-lal model
      - same CLS pooling via benchmark_aiteamvn_holdouts.encode_cls
      - query batch = 1
      - top_passages(..., count=2)
      - max_length = 512
      - max passage score per document

    Only the passage micro-batch is increased for throughput. If CUDA OOM
    occurs, automatically fall back to smaller batches without changing the
    scoring contract.
    """
    from transformers import AutoModel, AutoTokenizer
    from benchmark_aiteamvn_holdouts import encode_cls
    ...
    path = cache_dir / "vnlegal_scores.pkl"
    ...
    print(
        f"vnlegal-lal cache {len(ids)-len(remaining)}/{len(ids)}",
        flush=True,
    )
    if not remaining:
        return saved
    ...
    print(
        ...
        f"| requested passage batch={passage_batch_size}",
        flush=True,
    )

    # Candidate fallback ladder. Keep 64 as the historical batch before
    # becoming more conservative.
    ladder = []
    for b in (passage_batch_size, 96, 80, 64, 48, 32, 16):
        if b > 0 and b not in ladder and b <= passage_batch_size:
            ladder.append(b)
    if 64 not in ladder and passage_batch_size > 64:
        ladder.append(64)
    ladder = sorted(set(ladder), reverse=True)

    active_batch = ladder[0]
    started = time.perf_counter()

    for i, q in enumerate(remaining, 1):
        question = questions[q]

        # Preserve historical query batching exactly.
        ...
        if passages:
            while True:
                try:
                    ...
                    torch.cuda.empty_cache()
                    smaller = [b for b in ladder if b < active_batch]
                    if not smaller:
                        raise
                    next_batch = smaller[0]
                    print(
                        f"  CUDA OOM at passage batch={active_batch}; "
                        f"falling back to {next_batch}",
                        flush=True,
                    )
...
def preflight_future_stages(root: Path, sibling: Path, device: str):
    """
    Fail-fast audit for Stage 6 -> Stage 12.

    This runs BEFORE expensive private inference. It intentionally checks
    dependencies that used to fail only after E5 / reranker work had already
    consumed substantial time.

    It performs no private-label access and no private inference.
    """
    print("[PRE/12] Fail-fast audit for Stage 6 -> Stage 12...", flush=True)
    ...
    rel_required = {
        "REL base script": base_script,
        "REL chunk embeddings": bundle / "embeddings.f16.npy",
        "REL chunk IDs": bundle / "chunk_ids.jsonl",
        "REL VietLegal-E5 model": rel_model_dir,
        "REL Fold0 CE checkpoint": checkpoint,
        "REL evidence DB": evidence_db,
    }
    for label, p in rel_required.items():
        if not p.exists():
            errors.append(f"Missing REL dependency [{label}]: {p}")

    # exp_final can be in sibling/exp_final or sibling/src/exp_final.
    for module_file in ("cross_encoder.py", "evidence.py"):
        candidates = [
            sibling / "exp_final" / module_file,
            sibling / "src" / "exp_final" / module_file,
        ]
        if not any(p.is_file() for p in candidates):
            errors.append(
                f"Missing exp_final/{module_file}; checked: "
                + ", ".join(str(p) for p in candidates)
            )

    # Validate the evidence matrix shape before waiting for Stage 10.
    emb_path = bundle / "embeddings.f16.npy"
    if emb_path.is_file():
        try:
            arr = np.load(emb_path, mmap_mode="r")
            if arr.shape != (EXPECTED_EVIDENCE_CHUNKS, E5_DIM):
                errors.append(
                    f"REL embedding shape drift: {arr.shape} != "
                    f"({EXPECTED_EVIDENCE_CHUNKS}, {E5_DIM})"
                )
            if arr.dtype != np.float16:
                errors.append(
                    f"REL embedding dtype drift: {arr.dtype} != float16"
                )
            del arr
        except Exception as exc:
            errors.append(f"REL embedding mmap failed: {exc}")

    # Validate chunk inventory population.
    chunk_ids_path = bundle / "chunk_ids.jsonl"
    if chunk_ids_path.is_file():
        try:
            ...
            if chunk_n != EXPECTED_EVIDENCE_CHUNKS:
                errors.append(
                    f"REL chunk count drift: {chunk_n} != "
                    f"{EXPECTED_EVIDENCE_CHUNKS}"
                )
            if len(parent_ids) != EXPECTED_EVIDENCE_DOCS:
                errors.append(
                    f"REL parent-doc count drift: {len(parent_ids)} != "
                    f"{EXPECTED_EVIDENCE_DOCS}"
                )
            notes["rel_chunks"] = chunk_n
            notes["rel_parent_docs"] = len(parent_ids)
        except Exception as exc:
            errors.append(f"REL chunk inventory audit failed: {exc}")

    # Validate evidence DB schema/signature now.
    if evidence_db.is_file():
        try:
            ...
            try:
                ...
                if row is None:
                    errors.append("REL evidence DB inventory signature missing")
                chunk_probe = con.execute(
                    "SELECT COUNT(*) FROM chunks"
                ).fetchone()
                notes["evidence_db_chunks"] = (
                    int(chunk_probe[0]) if chunk_probe else None
                )
            finally:
                con.close()
        except Exception as exc:
            errors.append(f"REL evidence DB audit failed: {exc}")

    # Most importantly: derive the frozen REL threshold NOW. This catches
    # missing Fold0 score files / provenance drift before Stage 6 GPU work.
    rel_contract = None
    if not errors:
        try:
            rel_contract = derive_rel_l0(root, sibling)
            notes["rel_contract"] = rel_contract
        except Exception as exc:
            errors.append(f"REL_L0 derivation preflight failed: {exc}")
    ...
    if errors:
        ...
        raise RuntimeError(
            f"Future-stage preflight found {len(errors)} blocking issue(s). "
            "Nothing expensive was started."
        )

    print(
        "  PRE-FLIGHT PASS: Stage 6b-6f models, exact E5 provenance, "
        "REL bundle/checkpoint/evidence, and frozen REL_L0 contract are ready.",
        flush=True,
    )
    if notes.get("cuda_device"):
        print(f"  CUDA: {notes['cuda_device']}", flush=True)
    print(
        f"  REL inventory: chunks={notes.get('rel_chunks')} "
        f"parents={notes.get('rel_parent_docs')} "
        f"DB_chunks={notes.get('evidence_db_chunks')}",
        flush=True,
    )
...
def main():
    ...
    ap.add_argument("--vnlegal-batch-size", type=int, default=96, help="Passage batch for vnlegal-lal; auto-falls back on CUDA OOM.")
    ...
    preflight = preflight_future_stages(root, sibling, args.device)
    ...
    vnlegal_scores = score_vnlegal_channel(
        ...
        passage_batch_size=args.vnlegal_batch_size,
    )
    ...
    print("[8/12] Reusing preflight-verified frozen REL_L0 contract...", flush=True)
    rel_contract = preflight["rel_contract"]
    ...
    print(
        "NOTE: CPU base uses the restored exact historical generator contract; "
        "public ordered Top20 parity is hard-gated at 1000/1000."
    )
    print("=" * 112)
