...
def load_pickle(path: Path):
    # Historical XGBoost artifact was pickled under an older runtime. The
    # mandatory public hard gate reproduces cpu_top20 exactly 1000/1000, so
    # this persistence warning is known-safe for this frozen inference task.
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
            message=r"(?s).*loading a serialized model.*older version of XGBoost.*",
            category=UserWarning,
        )
...
def score_historical_e5_channel(
    ...
    Only CPU-base top20 docs are scored. This is intentional: public
    gpu_scores.checkpoint.pkl contains E5 scores only for those documents.
    Expanded/corpus-only D1 candidates remain missing in this channel and are
    filled by ltr_features at mean-2*std.
    """
    from transformers import AutoModel, AutoTokenizer
    from benchmark_jina_reranker_holdouts import top_passages

    model_path = root / HIST_GPU_E5["model"]
    if not model_path.is_dir():
        raise FileNotFoundError(
            f"Historical E5-small model missing: {model_path}"
        )

    # Prefer the original benchmark_e5_holdouts.encode implementation if the
    # restored source exists locally.
    try:
        from benchmark_e5_holdouts import encode as historical_encode
        encode_impl = historical_encode
        encode_name = "benchmark_e5_holdouts.encode"
    except Exception as exc:
        raise RuntimeError(
            "Historical generator imports benchmark_e5_holdouts.encode, but "
            "that module/function is unavailable locally. Restore the original "
            f"benchmark_e5_holdouts.py before continuing. Import error: {exc}"
        )
    ...
    for i, q in enumerate(remaining, 1):
        ...
        if i % 25 == 0 or i == len(remaining):
            ...
            print(
                ...
                f"{(time.perf_counter()-started)/i:.2f}s/q",
                flush=True,
            )
