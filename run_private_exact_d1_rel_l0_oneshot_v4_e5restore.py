...
def _standard_e5_encode(
    ...
    """
    Standard E5 encoder fallback:
      attention-mask mean pooling + L2 normalization.

    Used only when the historical benchmark_e5_holdouts.encode helper is not
    available locally. The public gpu_scores parity gate below then verifies
    this implementation against the historical cached E5 scores.
    """
    import torch.nn.functional as F
    ...
    for start in range(0, len(texts), batch_size):
        ...
        emb = hidden.sum(dim=1) / enc["attention_mask"].sum(
            dim=1, keepdim=True
        ).clamp(min=1)
...
def ensure_historical_e5_small(root: Path) -> Path:
    ...
    try:
        ...
        raise RuntimeError(
            "huggingface_hub is required to restore multilingual-e5-small. "
            f"Import failed: {exc}"
        )
...
def _historical_e5_encoder():
    """
    Prefer the historical local helper. Fall back to the canonical E5 encoder;
    the public cached-score gate decides whether the fallback is equivalent.
    """
    try:
        from benchmark_e5_holdouts import encode as historical_encode
        print(
            "  E5 encoder: benchmark_e5_holdouts.encode (historical helper)",
            flush=True,
        )
        return historical_encode, "benchmark_e5_holdouts.encode"
    except Exception as exc:
        print(
            "  benchmark_e5_holdouts.encode unavailable; "
            f"using standard E5 fallback ({exc})",
            flush=True,
        )
        return _standard_e5_encode, "standard_e5_mean_pool_l2"


def validate_historical_e5_public_scores(
    root: Path,
    model,
    tokenizer,
    encode_impl,
    ...
    """
    Verify model + tokenizer + encode implementation against historical
    gpu_scores.checkpoint.pkl before scoring private queries.

    We compare raw document E5 scores for deterministic public queries.
    """
    from benchmark_jina_reranker_holdouts import top_passages
    ...
    if not hist_path.is_file():
        raise FileNotFoundError(
            f"Historical GPU score cache missing: {hist_path}"
        )
    if not cpu_path.is_file():
        raise FileNotFoundError(
            f"Historical CPU top20 missing: {cpu_path}"
        )
    ...
    if len(sample_ids) < min(sample_n, 3):
        raise RuntimeError(
            f"Historical E5 public parity sample too small: {len(sample_ids)}"
        )
    ...
    for q in sample_ids:
        ...
        owners, passages = [], []
        for d in cpu[q][:HIST_GPU_E5["candidate_depth"]]:
        ...
        calc = {d: -1.0 for d in cpu[q][:20]}
        for d, score in zip(owners, pv @ qv):
            calc[d] = max(calc[d], float(score))

        old = historical[q]["e5"]
        common = [d for d in cpu[q][:20] if d in old]
        if len(common) != 20:
            raise RuntimeError(
                f"Historical E5 cache qid={q} has only "
                f"{len(common)}/20 base docs"
            )

        diff = max(abs(calc[d] - float(old[d])) for d in common)
        max_abs = max(max_abs, diff)
        compared += len(common)

        calc_rank = sorted(common, key=lambda d: (-calc[d], d))
        old_rank = sorted(common, key=lambda d: (-float(old[d]), d))
        exact_rank += int(calc_rank == old_rank)
        top5_rank += int(calc_rank[:5] == old_rank[:5])

    print(
        f"  PUBLIC E5 RESTORE PARITY: queries={len(sample_ids)} "
        f"docs={compared} | exact E5 order={exact_rank}/{len(sample_ids)} "
        f"| exact top5={top5_rank}/{len(sample_ids)} "
        f"| max_abs_diff={max_abs:.8f}",
        flush=True,
    )

    # Same GPU/model path should normally be much tighter than this. We allow
    # a small tolerance for fp16/runtime differences but require ranking parity.
    if top5_rank != len(sample_ids) or max_abs > 0.003:
        raise RuntimeError(
            "RESTORED E5-SMALL FAILED HISTORICAL PUBLIC SCORE PARITY: "
            f"top5={top5_rank}/{len(sample_ids)}, "
            f"max_abs_diff={max_abs:.8f}. "
            "Do not score private E5 with this model/encoder contract."
        )

    return {
        "status": "PASS_HISTORICAL_GPU_E5_SAMPLE",
        "queries": len(sample_ids),
        "docs": compared,
        "exact_order": exact_rank,
        "exact_top5": top5_rank,
...
def score_historical_e5_channel(
    ...
    encode_impl, encode_name = _historical_e5_encoder()
    ...
    # One-time historical score gate before private inference.
    e5_gate = validate_historical_e5_public_scores(
        root,
        model,
        tok,
        encode_impl,
