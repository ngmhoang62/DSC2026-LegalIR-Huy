...
def _encode_e5_variant(
    ...
    *,
    ...
    """Encode one deterministic E5 contract candidate."""
    import torch.nn.functional as F
    ...
    for start in range(0, len(texts), batch_size):
        ...
        if normalize_dtype == "float32":
            emb = emb.float()
        elif normalize_dtype == "model":
            pass
        else:
            raise ValueError(normalize_dtype)
...
def _make_e5_encoder(pooling, normalize_dtype, do_normalize=True):
    def encode(model, tokenizer, texts, batch_size, max_length):
        return _encode_e5_variant(
            model,
            tokenizer,
            texts,
            batch_size,
    ...
    encode.__name__ = (
        f"e5_{pooling}_{normalize_dtype}_"
        f"{'norm' if do_normalize else 'raw'}"
    )
    return encode
...
def ensure_historical_e5_small(root: Path) -> Path:
    ...
    pinned Hugging Face revision to the historical local path.
    """
    ...
    for p in sibling_candidates:
        ...
        if valid_model_dir(p):
            print(f"  historical E5-small resolved -> {p}", flush=True)
            return p
    ...
    if not valid_model_dir(local):
        raise RuntimeError(
            f"E5-small download completed but model directory is incomplete: {local}"
        )
...
def _load_historical_e5_probe_inputs(root: Path, documents, sample_n: int):
    """Load deterministic public oracle rows for E5 contract probing."""
    hist_path = root / "results/burst_gpu_threeview/gpu_scores.checkpoint.pkl"
    cpu_path = root / "results/burst_gpu_threeview/cpu_top20.pkl"
    public_path = (
        root
    ...
    if not hist_path.is_file():
        raise FileNotFoundError(f"Historical GPU cache missing: {hist_path}")
    if not cpu_path.is_file():
        raise FileNotFoundError(f"Historical CPU top20 missing: {cpu_path}")

    hist_obj = load_pickle(hist_path)
    historical = hist_obj.get("scores", hist_obj)
    cpu = load_pickle(cpu_path)["rankings"]
    public_ids, public_questions = load_questions(public_path)

    eligible = [
        q for q in public_ids
        if q in historical
        and q in cpu
        and historical[q].get("e5")
    ]
    if len(eligible) < sample_n:
        raise RuntimeError(
            f"Historical E5 oracle has only {len(eligible)} eligible queries"
        )

    # Spread probe queries across the 1000-query sequence instead of only
    # taking the first N, to reduce accidental easy-case selection.
    idx = np.linspace(0, len(eligible) - 1, sample_n, dtype=int)
    sample_ids = [eligible[i] for i in idx]

    return historical, cpu, public_questions, sample_ids


def _evaluate_e5_encoder_contract(
    root: Path,
    model,
    tokenizer,
    encode_impl,
    ...
    *,
    ...
    historical, cpu, public_questions, sample_ids = (
        _load_historical_e5_probe_inputs(root, documents, sample_n)
    )
    ...
    for q in sample_ids:
        ...
        base_docs = cpu[q][:HIST_GPU_E5["candidate_depth"]]
        ...
        old = historical[q]["e5"]
        common = [d for d in base_docs if d in old]
        if len(common) != len(base_docs):
            raise RuntimeError(
                f"Historical E5 cache qid={q} incomplete: "
                f"{len(common)}/{len(base_docs)}"
            )

        a = np.asarray([calc[d] for d in common], dtype=np.float64)
        b = np.asarray([float(old[d]) for d in common], dtype=np.float64)
        diffs.extend((a - b).tolist())

        if np.std(a) > 0 and np.std(b) > 0:
            pearsons.append(float(np.corrcoef(a, b)[0, 1]))

        calc_rank = sorted(common, key=lambda d: (-calc[d], d))
        old_rank = sorted(common, key=lambda d: (-float(old[d]), d))
        exact_order += int(calc_rank == old_rank)
        exact_top5 += int(calc_rank[:5] == old_rank[:5])
        exact_top10 += int(calc_rank[:10] == old_rank[:10])
    ...
    return {
        ...
        "mean_signed_diff": float(np.mean(diffs)),
        "pearson": float(np.mean(pearsons)) if pearsons else None,
    }


def resolve_historical_e5_encoder(
    root: Path,
    model,
    tokenizer,
    documents,
    ...
    """
    Resolve the missing benchmark_e5_holdouts.encode contract by direct
    output matching against the historical gpu_scores.checkpoint.pkl.

    This uses no labels. The historical raw E5 score cache is the oracle.
    """
    candidates = []

    # If Huy restores the original helper later, it gets first priority.
    try:
        from benchmark_e5_holdouts import encode as original_encode
        candidates.append(("historical_helper", original_encode))
    except Exception:
        pass

    # Plausible contracts based on this repo's encoder conventions.
    candidates.extend([
        (
            "cls_float32_norm",
            _make_e5_encoder("cls", "float32", True),
        ),
        (
            "mean_float32_norm",
            _make_e5_encoder("mean", "float32", True),
        ),
        (
            "cls_modeldtype_norm",
            _make_e5_encoder("cls", "model", True),
        ),
        (
            "mean_modeldtype_norm",
            _make_e5_encoder("mean", "model", True),
        ),
        # Raw variants are unlikely for E5 but cheap enough to falsify.
        (
            "cls_float32_raw",
            _make_e5_encoder("cls", "float32", False),
        ),
        (
            "mean_float32_raw",
            _make_e5_encoder("mean", "float32", False),
        ),
    ])

    print(
        "  Probing missing historical benchmark_e5_holdouts.encode "
        "against gpu_scores.checkpoint.pkl...",
        flush=True,
    )

    trials = []
    for name, fn in candidates:
        metrics = _evaluate_e5_encoder_contract(
            root,
            model,
            tokenizer,
            fn,
        ...
        trials.append((name, fn, metrics))
        print(
            f"    {name:22s} "
            f"top5={metrics['exact_top5']:2d}/{metrics['queries']} "
            f"top10={metrics['exact_top10']:2d}/{metrics['queries']} "
            f"all={metrics['exact_order']:2d}/{metrics['queries']} "
            f"MAE={metrics['mean_abs_diff']:.8f} "
            f"MAX={metrics['max_abs_diff']:.8f} "
            f"r={metrics['pearson']:.8f}",
            flush=True,
        )

    # Ranking parity dominates because downstream D1 only uses standardized
    # score geometry; raw score error breaks ties between ranking-equivalent
    # candidates and is therefore used as the second key.
    trials.sort(
        key=lambda x: (
            x[2]["exact_top5"],
            x[2]["exact_top10"],
    ...
    best_name, best_fn, best_metrics = trials[0]

    print(
        f"  BEST HISTORICAL E5 CONTRACT: {best_name} | "
        f"top5={best_metrics['exact_top5']}/{best_metrics['queries']} "
        f"MAE={best_metrics['mean_abs_diff']:.8f} "
        f"MAX={best_metrics['max_abs_diff']:.8f}",
        flush=True,
    )

    # Accept only a genuinely convincing historical reconstruction.
    # If CLS is the missing helper, we expect much tighter raw parity than the
    # failed mean-pool fallback (~0.024 max error).
    if (
        best_metrics["exact_top5"] != best_metrics["queries"]
        or best_metrics["exact_top10"] != best_metrics["queries"]
        or best_metrics["mean_abs_diff"] > 0.003
        or best_metrics["max_abs_diff"] > 0.01
    ):
        raise RuntimeError(
            "NO E5 ENCODER CONTRACT REPRODUCES THE HISTORICAL CACHE "
            "TIGHTLY ENOUGH. Please restore/upload benchmark_e5_holdouts.py. "
            f"Best={best_name}, metrics={best_metrics}"
        )

    return best_fn, best_name, best_metrics
...
def score_historical_e5_channel(
    ...
    encode_impl, encode_name, e5_gate = resolve_historical_e5_encoder(
        root,
        model,
        tok,
        documents,
    ...
    print(
        f"  historical E5 encoder selected: {encode_name}",
        flush=True,
    )
...
def main():
    ...
    print("[0/12] HARD GATE: restored historical CPU public parity...", flush=True)
