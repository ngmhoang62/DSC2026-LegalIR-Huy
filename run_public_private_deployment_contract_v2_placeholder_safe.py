...
def main():
    ...
    def nonempty_label_payload(v):
        ...
        for k in ("answer", "answers", "gold", "label"):
            if k not in v:
                continue
            x = v[k]
            # Empty placeholders (None, "", [], {}, ()) are metadata/schema,
            # not leaked labels.
            if x is None or x == "" or x == [] or x == {} or x == ():
                continue
            return True
        return False

    public_label_like_fields_present=[
        q for q,v in pub.items()
        if isinstance(v,dict) and any(k in v for k in ("answer","answers","gold","label"))
    ]
    public_nonempty_label_payloads=[
        q for q,v in pub.items()
        if nonempty_label_payload(v)
    ]
    ...
    pass_contract=(
        not overlap
        and not public_nonempty_label_payloads
    ...
    report={
        ...
        "population":{
            ...
            "public_records_with_label_like_fields":public_label_like_fields_present,
            "public_records_with_nonempty_label_payloads":public_nonempty_label_payloads,
    ...
    print(
        ...
        f"public label-like fields={len(public_label_like_fields_present)}; "
        f"nonempty label payloads={len(public_nonempty_label_payloads)}; "
        f"valid_docs={len(valid_docs)}"
    )
    print(
