#!/usr/bin/env python
"""
D1 RANK6-10 MULTI-EXPERT + CHALLENGER-ONLY CE CERTIFICATE — CAL V1
===========================================================================
...
Test ONE preregistered recall-rescue policy on exact D1 LOBO:
  ...
  challenger ∈ exact D1 ranks {6,7,8,9,10}
...
def score_rank610_bge(
    *,
    ...
    score_dir = output_dir / "bge_rank6_10_scores"
        ...
        for i, q in enumerate(all_ids, 1):
            ...
            if cache_path.is_file():
                ...
                if obj.get("signature") != sig:
                    raise RuntimeError(f"BGE rank6-10 cache drift q={q}")
                ch_scores = {
                    str(d): float(s)
...
def main():
    ...
    print("[2/6] Loading frozen expert caches and sealing LABEL-FREE prefilter...")
    ...
    # Challenger prefilter uses the SAME thresholds as the previous preregistered
    # rank6-10 arm. Only the defender-weakness gate is removed, and the natural
    # D1 boundary window is expanded to ranks 6-10.
    prefilter = {}
    prefilter_diag = {}
    for q in ids:
        top = rankings[q]
        ...
        for rank in range(6, MAX_CHALLENGER_RANK + 1):
            ...
            rows.append({
                ...
                "prefilter_pass": bool(pre),
            })
            if pre:
                qualified.append(c)

        prefilter[q] = qualified
        prefilter_diag[q] = rows
    ...
    print("[3/6] Loading sanitized V2 renderer and scoring ONLY prefiltered rank6-10 challengers...")
    m = loadmod(base_script, "cebase_rank610_challenger_cert")
    cal_ids, _ = m.get_cal_ids_label_free(root)
    v2 = m.load_noncal_world(root, sibling, set(cal_ids))
    bge, bge_abstain, ckpt = score_rank610_bge(
        root=root,
        sibling=sibling,
        world=v2,
        all_ids=ids,
    ...
    for q in ids:
        ...
        for rank in range(6, MAX_CHALLENGER_RANK + 1):
            ...
            cr = {
                ...
                "external_votes": prow["external_votes"],
                "external_vote_count": prow["external_vote_count"],
                "rankview_votes": prow["rankview_votes"],
                "rankview_vote_count": prow["rankview_vote_count"],
    ...
    sealed = {
        "schema": "manual.d1_rank610_multiexpert_challenger_cert_v1.actions",
    ...
    verdict = (
        ...
        else "KILL_RANK610_CHALLENGER_CERT_POLICY"
    )

    report = {
        "schema": "manual.d1_rank610_multiexpert_challenger_cert_v1.report",
        "verdict": verdict,
