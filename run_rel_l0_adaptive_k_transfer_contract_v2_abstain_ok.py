...
def main():
    ...
    # Missing CE score rows are an allowed conservative abstention: the
    # deployed policy keeps exact D1 unchanged on those queries. This is the
    # intended contract for V2/evidence coverage gaps, not a transfer failure.
    abstention_contract_ok = all(candidate[q] == d1[q] for q in missing)
    ...
    report={
        ...
        "cal":{
            "queries":len(ids),"scorable_rows":len(rows),
            "abstained_unscorable_qids":missing,
            "abstention_contract_ok":abstention_contract_ok,
