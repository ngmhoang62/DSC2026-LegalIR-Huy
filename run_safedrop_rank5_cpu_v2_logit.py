...
def safe_probability_threshold(scores, labels, fin):
    """Calibrate in logit space so a safety margin remains inside (0,1)."""
    ...
    pos_logits = _logit(positives)
    threshold_logit = (
        float(np.min(pos_logits))
        - META_MARGIN * fin.robust_scale(pos_logits)
    )
    return float(_sigmoid(threshold_logit))
...
def main():
    ...
    for outer_held in sorted(blocks):
        ...
        for meta_held in outer_train_blocks:
            ...
            for q, a, b in zip(meta_test_ids, p_lr, p_hgb):
                meta_oof_lr[q] = float(a)
                meta_oof_hgb[q] = float(b)

        oof_lr = np.asarray(
            [meta_oof_lr[q] for q in outer_train_ids], dtype=np.float64
        )
        oof_hgb = np.asarray(
            [meta_oof_hgb[q] for q in outer_train_ids], dtype=np.float64
        )
        oof_labels = np.asarray(
            [y_train[q] for q in outer_train_ids], dtype=np.int8
        )

        lr_threshold = safe_probability_threshold(
            oof_lr, oof_labels, fin
        )
        hgb_threshold = safe_probability_threshold(
            oof_hgb, oof_labels, fin
        )
        if lr_threshold is None or hgb_threshold is None:
            raise RuntimeError(
                f"No gold rank5 in outer-train for held={outer_held}"
            )

        # Dual veto: BOTH separately calibrated experts must say safely non-relevant.
        cal_drop = (
            (oof_lr <= lr_threshold)
            & (oof_hgb <= hgb_threshold)
        )
        ...
        for q, a, b, j in zip(
            ...
            meta_safe = (
                float(a) <= lr_threshold
                and float(b) <= hgb_threshold
            )
        ...
        outer_reports[outer_held] = {
            ...
            "lr_threshold_logit_calibrated": float(lr_threshold),
            "hgb_threshold_logit_calibrated": float(hgb_threshold),
            ...
            "meta_calibration_oof": {
                ...
                "gold_min_lr_probability": float(
                    np.min(oof_lr[oof_labels == 1])
                ),
                "gold_min_hgb_probability": float(
                    np.min(oof_hgb[oof_labels == 1])
                ),
            },
            "primary_outer": {
        ...
        print(
            ...
            f"lr_thr={lr_threshold:.4f} hgb_thr={hgb_threshold:.4f} "
            f"scalar_thr={scalar_threshold:.4f} | "
    ...
    report = {
        "schema": "manual.safedrop_rank5_cpu_v2_logit",
        "status": "COMPLETE",
        "protocol": {
            "outer": "4-block LOBO",
            ...
            "dual_veto": (
                "P_rel_LR <= LR_threshold AND "
                "P_rel_HGB <= HGB_threshold"
            ),
            "probability_calibration_space": "logit",
            "meta_safety_margin": META_MARGIN,
            "primary_gate": (
                "dual_veto AND ROBUST_Z5 <= lambda0.25_threshold"
            ),
            "gpu_inference": False,
    ...
    out = root / "results/manual/huy_safedrop_rank5_cpu_v2_logit"
