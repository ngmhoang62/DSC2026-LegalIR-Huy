...
def main():
    ...
    from benchmark_burst_v4_full_sqlite import (
        retrieve_docs, retrieve_local, fuse, build_database, load_dataset
    )
    ...
    primary_db=root/"benchmarks/legalir_full_fts.sqlite"
    sibling_db=root.parent/"LegalIR/benchmarks/legalir_full_fts.sqlite"
    audit_dir=root/"results/manual/huy_sparse_fts_query_tokenization_v2"
    audit_dir.mkdir(parents=True,exist_ok=True)
    audit_db=audit_dir/"legalir_full_fts_audit.sqlite"
    ...
    if primary_db.is_file():
        ...
        db_source="PRIMARY_EXISTING"
    elif sibling_db.is_file():
        db=sibling_db
        conn=sqlite3.connect(f"file:{db.resolve().as_posix()}?mode=ro",uri=True)
        db_source="SIBLING_EXISTING"
    else:
        db=audit_db
        print(
            "  no historical FTS DB found; building audit-only exact FTS5 index "
            "(chunk_size=500, overlap=100)...",
            flush=True,
        )
        conn,_=build_database(db,docs_all,chunk_size=500,overlap=100)
        db_source="AUDIT_REBUILT_FROM_8532_CONTEXTS"

    print(f"  FTS DB source={db_source}: {db}",flush=True)
    ...
    report={
        "schema":"manual.sparse_fts_query_tokenization_v2",
        "population":{
            "queries":len(ids),
        ...
        "fts_provenance":{
            ...
            "chunk_size":500,
            "overlap":100,
            "tokenizer":"SQLite FTS5 unicode61; Python query TOKEN_RE=\\w+",
        },
    }
    ...
    out=root/"results/manual/huy_sparse_fts_query_tokenization_v2"
