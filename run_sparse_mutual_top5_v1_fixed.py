...
def reconstruct_d1(root: Path):
    ...
    queries, blocks_raw, all_ids, extended, local_views, base_scores = build_training_cap(
        root,
        32,
    ...
    # Canonicalize block keys once. Historical loaders may return
    # {'a','b','c','d'} while our reporting/parity contract uses
    # {'A','B','C','D'}. Keep ONE representation downstream.
    blocks = {str(k).upper(): list(v) for k, v in blocks_raw.items()}
    expected_block_keys = {"A", "B", "C", "D"}
    if set(blocks) != expected_block_keys:
        raise RuntimeError(
            "Unexpected block keys from build_training_cap: "
            f"raw={list(blocks_raw.keys())}, normalized={list(blocks.keys())}"
        )
...
def main():
    ...
    d1_parity = {
        "status": "PASS",
        "canonical_block_keys": list(d1["blocks"].keys()),
    ...
    block_deltas = {
        str(b).upper(): (
            cand_m["block_recalls"][str(b).upper()]
            - baseline_m["block_recalls"][str(b).upper()]
        )
        for b in d1["blocks"]
    }
