...
def walk_roots(root: Path, extra: list[Path]):
    roots = []
    for p in [
        root / "results",
        root / "cache",
...
def main():
    ap = argparse.ArgumentParser(
        description="Read-only inventory of ignored/local caches likely relevant to LegalIR reproduction."
    )
    ap.add_argument("--repo-root", type=Path, required=True)
    ap.add_argument(
        "--extra-root", action="append", default=[],
        help="Additional cache/result root to scan, e.g. /d/Study/DSC2026/LegalIR/cache",
    )
    ap.add_argument(
    ...
    extra = [Path(x) for x in args.extra_root]
    ...
    for scan_root in walk_roots(root, extra):
