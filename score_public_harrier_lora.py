"""Score the PUBLIC pool with vietlegal-harrier-0.6b + the supplied LoRA adapter.
...
def main():
    ...
    ap.add_argument("--out", default="results/from_drive/harrier_ft_public.pkl")
    ...
    pub = json.loads((data / "public-official.json").read_text(encoding="utf-8"))
    qtext = {q: (v["question"] if isinstance(v, dict) else v) for q, v in pub.items()}
    pool = pickle.loads(
        (root / "results/crossenc_fullpool/public_scores.pkl").read_bytes())["scores"]
    all_ids = [q for q in qtext if q in pool]
    extended = {q: list(pool[q]) for q in all_ids}
    queries = {q: (qtext[q],) for q in all_ids}
