...
INCLUDES the 600-query LOBO block, so the per-epoch evaluate() is scoring on
queries the model was also trained on and its numbers are optimistic, not a
generalization estimate.  Loss is InfoNCE over one gold document against hard
