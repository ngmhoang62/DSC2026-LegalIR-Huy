"""Stage 2: Verify Public D1 Control Parity against Champion Submission (1000/1000 matches required)."""
...
def run_public_d1_control_parity() -> Tuple[Dict[str, Any], Dict[str, List[str]], Dict[str, Dict[str, float]], Dict[str, List[str]], Any, Dict[str, str], List[str]]:
    print("=== STAGE 2: PUBLIC D1 CONTROL PARITY AUDIT ===", flush=True)
    ...
    docs_store = DocumentStore(
        sorted(PUBLIC_CONTEXTS_DIR.glob("context_*.json"))
    )
    ...
    paths_meta, doc_ids_meta, train_meta, public_meta = load_metadata(PUBLIC_DATASET_DIR)
    public_ids = list(public_meta)
    valid_docs = set(doc_ids_meta)

    if len(public_ids) != EXPECTED_PUBLIC_QUERIES:
        raise ValueError(f"Expected {EXPECTED_PUBLIC_QUERIES} public queries, got {len(public_ids)}")
    ...
    # 3. Train D1 on full CAL600
    print("Training production D1 learner on full CAL600...", flush=True)
    docs_cal, queries_cal, blocks_cal, all_ids_cal, extended_cal, local_views_cal, full_channels_cv, type_rows_cal, cite_rows_cal = load_cal_data_label_free()
    gold_cal, _ = load_cal_gold_labels(all_ids_cal)
    ...
    d1_views_cal = dict(local_views_cal)
    d1_rows_cal_base, d1_groups = ltr_features(
        d1_views_cal, D1_VIEWS, extended_cal, all_ids_cal, full_channels_cv
    )
    d1_rows_cal = {
        q: np.concatenate([d1_rows_cal_base[q], type_rows_cal[q], cite_rows_cal[q]], axis=1)
        for q in all_ids_cal
    }
    feature_dim = d1_rows_cal[all_ids_cal[0]].shape[1]

    X_train_full = np.vstack([d1_rows_cal[q] for q in all_ids_cal])
    y_train_full = np.concatenate(
        [[d in gold_cal[q] for d in d1_groups[q]] for q in all_ids_cal]
    ).astype(np.int8)

    scaler_d1 = StandardScaler().fit(X_train_full)
    ...
    model_d1.fit(scaler_d1.transform(X_train_full), y_train_full)

    # Predict on Public Test
    print("Running production inference for D1 control on public queries...", flush=True)
    d1_rows_pub_base, _ = ltr_features(
        view_rank_pub, D1_VIEWS, public_candidates, public_ids, public_scores_base
    )
    d1_rows_pub = {
        q: np.concatenate([d1_rows_pub_base[q], public_t_rows[q], public_c_rows[q]], axis=1)
        for q in public_ids
    }
    ...
    for q in public_ids:
        X_q = scaler_d1.transform(d1_rows_pub[q])
        dec = model_d1.decision_function(X_q)
    ...
    # 4. Compare vs D1 Champion Submission
    print(f"Comparing vs champion: {D1_CHAMPION_JSON_PATH}...", flush=True)
    champion_raw = json.loads(D1_CHAMPION_JSON_PATH.read_text(encoding="utf-8"))
    champion_preds = {
        q: [str(d) for d in champion_raw[q]["answer"][:5]]
        for q in public_ids
    }
    ...
    mismatches = []
    for q in public_ids:
        if preds_control_d1[q] == champion_preds[q]:
            exact_matches += 1
        else:
            mismatches.append({
                "qid": q,
                "reconstructed_top5": preds_control_d1[q],
                "champion_top5": champion_preds[q],
            })

    print(f"Public D1 Control Parity: {exact_matches}/{len(public_ids)} exact ordered matches.", flush=True)
    ...
    parity_doc = {
        "schema_version": "dsc2026.gemini.huy_d1_exact_citation_public_v1.public_d1_control_parity.v1",
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "git_commit": git_info["head_commit"],
        "status": "PASS" if exact_matches == len(public_ids) else "FAIL",
        "total_public_queries": len(public_ids),
        "exact_matches_count": exact_matches,
        "control_parity_passed": exact_matches == len(public_ids),
        "d1_feature_dimension": feature_dim,
    ...
    out_path = RESULTS_DIR / "PUBLIC_D1_CONTROL_PARITY.json"
    out_path.write_text(json.dumps(parity_doc, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"Wrote {out_path}", flush=True)

    return parity_doc, preds_control_d1, scores_control_d1, public_candidates, docs_store, public_meta, public_ids
