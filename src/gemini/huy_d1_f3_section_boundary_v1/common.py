"""Common paths, constants, loaders, and provenance for HUY_D1_F3_SECTION_BOUNDARY_V1."""
...
SRC_DIR = ROOT / "src/gemini/huy_d1_f3_section_boundary_v1"
RESULTS_DIR = ROOT / "results/gemini/huy_d1_f3_section_boundary_v1"

# Frozen evidence caches
CAL_FROZEN_SECTION_CACHE_PATH = ROOT / "results/gemini/huy_d1_legal_section_evidence_v1/legal_section_ce_cv.pkl"
F3_MODEL_PATH = ROOT / "cache/sol_high_rl/aiteam_ft_full_corpus/noncal_boundary_diag_model.npz"
F3_SCORES_PATH = ROOT / "cache/sol_high_rl/aiteam_ft_full_corpus/cal600_noncal_boundary_diag_scores.pkl"
F3_SOURCE_PATH = ROOT / "src/sol_high_rl/noncal_boundary_metric_dev.py"
F3_FEASIBILITY_PATH = ROOT / "results/sol_high_rl/BOUNDARY_SPECIALIST_FEASIBILITY.json"
...
EXPECTED_5FOLD_SPLIT_SHA256 = "c6010d4ee8ff9e7d05ec6f17245c107819a5fecf7cfbeae11b241ad1be3f47f3"
SEED = 2026
...
def evaluate_parity(
    ...
    """Verify exact parity of D1 baseline against frozen champion."""
    recalls = [
        len(set(d1_rankings[q][:5]) & gold[q]) / max(1, len(gold[q]))
        for q in all_ids
    ]
    ...
    r5_match = abs(mean_r5 - EXPECTED_D1_R5) < 1e-12
    block_matches = {
        b: abs(block_recalls[b] - EXPECTED_BLOCK_RECALLS[b]) < 1e-12
    ...
    parity_doc = {
        "schema_version": "dsc2026.gemini.huy_d1_f3_section_boundary_v1.d1_parity.v1",
        ...
        "block_recalls": {
            b: {
                "reproduced": block_recalls[b],
                "expected": EXPECTED_BLOCK_RECALLS[b],
                "match": block_matches[b],
            }
            for b in sorted(EXPECTED_BLOCK_RECALLS.keys())
        },
